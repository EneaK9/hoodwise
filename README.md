...
2. **Understand the question.** `app/understand.py` makes one structured-output model call (`UNDERSTAND_MODEL`, default `claude-opus-5`) with the message, the recent user turns, the VIN-pinned car if any, and the catalog of manuals we hold with their year ranges. It returns the part category, the car as stated in the text (make/model/year/engine/fuel, merged across turns), what is still missing, the one question to ask back with tap options, and a cleaned retrieval query. Any language, typos included. No keyword lists: `app/intent.py` and `app/clarify.py` are the regex fallback used only when no API key is set.
3. **Clarify before guessing.** If the understanding step reports something missing, the app asks instead of answering: which car, which year (offering the generations we hold), petrol or diesel and engine size for fluid questions, which lamp for bulbs, or which part when the message is too vague. Replies like "the model is hyundai santa fe 2016" or "2.0 diesel" are recognised as answers and merged into the pinned car. Definitions ("what does DPF mean") skip this.
